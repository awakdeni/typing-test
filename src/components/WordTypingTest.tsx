...
import { indonesianWords } from '@/data/indonesian_words';
import { Timer, RefreshCw, ChevronDown } from 'lucide-react';

const generateRandomText = (difficulty: string) => {
  const wordCount = 25;
  let words = [];
  
  for (let i = 0; i < wordCount; i++) {
    let word = indonesianWords[Math.floor(Math.random() * indonesianWords.length)];
...
export const WordTypingTest = () => {
  ...
  useEffect(() => {
    setCurrentText(generateRandomText(selectedDifficulty));
  }, []);
